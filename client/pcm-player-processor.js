...
class PCMPlayerProcessor extends AudioWorkletProcessor {
  ...
  // }
  process(inputs, outputs, parameters) {
    // We are a playback node, so we only need the output buffer.
    const outputChannel = outputs[0][0]; // Mono output

    for (let i = 0; i < outputChannel.length; i++) {
      // Check if there is data available to read.
      if (this.readIndex !== this.writeIndex) {
        // Data is available: read from the buffer and advance the read index.
        outputChannel[i] = this.buffer[this.readIndex];
        this.readIndex = (this.readIndex + 1) % this.bufferSize;
      } else {
        // Buffer is empty (underflow): write silence. THIS IS THE CRITICAL FIX.
        outputChannel[i] = 0;
      }
    }
