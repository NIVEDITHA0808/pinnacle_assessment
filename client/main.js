...
function sendBufferedAudio() {
    if (audioBuffer.length === 0 || !client.isConnected()) {
        return;
    }
    ...
    // Use your ApiClient to send the audio data
    // Note: Your client.sendAudio might need to be adjusted
    // if it doesn't already handle Base64 encoding.
    // Let's assume you need to create a helper for that.
    client.sendAudio(combinedBuffer.buffer);
...
function audioRecorderHandler(pcmData) {
    ...
    if (!bufferTimer) {
        // The interval can be adjusted. 200ms is a good starting point.
...
function stopRecording() {
    ...
    // we can send buffer audio, but since we stopped
    // no need
    sendBufferedAudio();
